...
namespace Block_Cipher_Modes_WPF
{
    public static class AESFunctions
    {
        ...
        private static byte[] AES_encrypt_block_with_IV(byte[] plainText, byte[] Key, byte[] IV)
        {
            byte[] output_buffer = new byte[plainText.Length];
        ...
        private static byte[] AES_Decrypt_block_with_IV(byte[] cipherText, byte[] Key, byte[] IV)
        {
            // Declare the string used to hold the decrypted text. 
            byte[] output_buffer = new byte[cipherText.Length];
